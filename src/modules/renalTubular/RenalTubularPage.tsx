...
export function RenalTubularPage() {
  ...
  const { snapshot, history, perturb, reset } = useEngineLoop(inputs, renalTubularLoopConfig);
  ...
  <Sparkline
    ...
    colorVar="var(--urine)"
  />
  <Sparkline label="ADH" unit="%" data={adhHistory} domainMin={0} domainMax={100} colorVar="var(--adh)" />
        </>
      }
