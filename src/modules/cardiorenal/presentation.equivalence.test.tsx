...
const derived: DerivedValues = {
  ...
  aldosterone: 0.3,
  baroreflexDrive: 0,
  effectiveHeartRate: 70,
  strokeVolume: 70,
  cardiacOutput: 4900,
  effectiveSVR: 1,
  meanArterialPressure: 93,
  renalAutoregulation: 1,
  renalBloodFlow: 0.5,
  filtrationFraction: 0.2,
  gfr: 100,
  reabsorptionFraction: 0.6,
  urineOutput: 100,
  netFluidBalance: 0,
};
...
const history: HistoryPoint[] = [
  { t: 0, map: 90, gfr: 95, bloodVolume: 99 },
  { t: 60, map: 93, gfr: 100, bloodVolume: 100 },
];

describe('cardiorenal presentation equivalence', () => {
  it('renders the schema diagram to the same DOM as the legacy diagram', () => {
    const classes = getDiagramClasses('cardiorenal');
    const presentation = buildCardiorenalPresentation({ state, derived, inputs, history, baselineHistory: null });
    const frame = presentation.diagram[0]!;

    const legacy = render(
      <ModuleShellProvider blinded={false}>
        <PhysiologyDiagram derived={derived} />
      </ModuleShellProvider>,
  ...
  it('renders the schema readouts to the same DOM as the legacy panel', () => {
    ...
    const legacy = render(
      <ModuleShellProvider blinded={false}>
        <ReadoutPanel state={state} derived={derived} inputs={inputs} />
      </ModuleShellProvider>,
