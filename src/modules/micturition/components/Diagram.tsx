...
/* The drawing was laid out on its own 200x220 canvas, with every coordinate and font size
 * hand-placed against it. Rather than re-tune all of them, it is scaled into the house
 * 560x440 frame as a whole: 1.6x puts the 7-unit labels at ~11, which is the size every
 * other diagram's label is. */
  ...
  return (
    ...
    >
      <g transform={FIT}>
        ...
        {/* Net flow. The arrow gives the direction; the number is what the phase label above
            cannot say — both used to read "filling", which said the same thing twice. */}
        {derived.netFlowRateMLperMin < -10 && (
          <text x="100" y="218" textAnchor="middle" className={styles.value} fontSize="8" fill="var(--o2)">
            ↓ {Math.abs(derived.netFlowRateMLperMin).toFixed(0)} mL/min
          </text>
        )}
        {derived.netFlowRateMLperMin > 0 && (
          <text x="100" y="218" textAnchor="middle" className={styles.value} fontSize="8" fill="var(--text)">
            ↑ {derived.netFlowRateMLperMin.toFixed(1)} mL/min
          </text>
        )}
